...
function adminState() {
  return {
    ...
    groqKeySet: !!CFG.groqKey, groqKeyMasked: mask(CFG.groqKey),
...
async function handleAdminApi(req, res, path, prefix = "/api/") {
  ...
  if (sub === "config" && req.method === "POST") {
    ...
    if (Array.isArray(patch.groqModels)) next.groqModels = patch.groqModels;
    for (const k of ["oblitToken", "adminPassword", "openrouterKey", "freeaiapikeyKey", "groqKey"])
