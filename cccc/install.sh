...
# `cccc` (bare) is now OWNED by the claudectl repo's installer — one consolidated
# tabbed app (Issues · Claude account pool · Board · …) that lands on Issues. We
# no longer emit a `cccc` chooser here (it fought claudectl's wrapper). This tool
# still ships the account subcommand wrappers below + the MCP/statusline. If a box
# has NO claudectl checkout, install cccc from there (devdashco/claudectl).
_wrapper cccp     panes
